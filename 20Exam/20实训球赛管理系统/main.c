...
int login(MYSQL* pconn,struct UserInfo* temp)
{
	char uname[30];
	int upwd;
...
int main()
{
	...
	if (temp.role == 1)
	{
		while (1) {
			...
			int choose;
			printf("请输入你要执行的操作：\n");
			printf("1.查看预约 2.添加球赛 3.查看用户预约的球赛 4.登记球员 5.登记球队 6.退出\n");
			scanf("%d", &choose);
	...
	if (temp.role == 2)
	{
		while (1) {
			...
			int choose;
			printf("请输入你要执行的操作：\n");
			printf("1.预约球赛 2.创建球赛 3.查询比赛 4.获取预约的球赛 5.登记球员 6.登记球队 7.权限管理 8.退出\n");
			scanf("%d", &choose);
			...
			case 1: AppointGame(pconn); break;
			case 2: CreatGame(pconn); break;
			case 3: GetGames(pconn); break;
			case 4: GetAppoint(pconn); break;
			case 5: RecordPlayer(pconn); break;
			case 6: RecordTeam(pconn); break;
			case 7: ChangeRole(pconn); break;
			case 8: return 0;
			}
		}
	}
